...
#### ✅ **sttn_lst_t** - All columns present in schema
...
#### ✅ **cnntr_lst_t** - All columns present in schema
- cnntr_id, sttn_id, cnntr_typ_cd, cnntr_nm_tx, pwr_tx
- is_avlbl_in, a_in, i_ts

#### ✅ **fvrt_lst_t** - All columns present in schema
- fvrt_id, usr_id, sttn_id, a_in, i_ts

#### ✅ **sssn_lst_t** - All columns present in schema
...
### Before (SQL Server):
```sql
SELECT TOP 1 * FROM otp_lst_t WHERE ... ORDER BY i_ts DESC
WHERE expry_ts > GETDATE()
```

### After (MySQL):
```sql
SELECT * FROM otp_lst_t WHERE ... ORDER BY i_ts DESC LIMIT 1
WHERE expry_ts > NOW()
```

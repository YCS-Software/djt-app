...
exports.getByIdMdl = function(data) {
    ...
    const QRY = `SELECT * FROM ( SELECT b.bkng_id AS id, b.bkng_cd AS code, u.nm_tx AS user, s.sttn_nm_tx AS station, b.bkng_dte AS date, b.bkng_tm AS time, b.durn_mnts_nbr AS durationMinutes, b.sttus_cd AS status FROM bkng_lst_t b LEFT JOIN usr_lst_t u ON u.usr_id=b.usr_id LEFT JOIN sttn_lst_t s ON s.sttn_id=b.sttn_id ORDER BY b.bkng_dte DESC, b.bkng_tm DESC ) q WHERE q.id = ${id}`;
    return dbutil.execQuery(sqldb.MySQLConPool, QRY, cntxtDtls);
};

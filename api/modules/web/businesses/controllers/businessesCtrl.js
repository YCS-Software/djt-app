/**
 * Web Businesses Controller
 * Admin web console (djt-web) — business organisations.
 * No backing table yet — list returns []; writes are no-ops.
 */

const std = require(appRoot + '/utils/standardMessages');
const mdl = require('../models/businessesMdl');

exports.list = function (req, res) {
    mdl.listMdl()
        .then(rows => res.status(200).json({ status: 200, rows: rows || [] }))
        .catch(e => { console.error('[businesses] list', e); res.status(500).json({ status: 500, error: 'Failed to load businesses' }); });
};

exports.get = function (req, res) {
    mdl.getByIdMdl({ id: req.params.id })
        .then(rows => res.status(200).json({ status: 200, row: (rows && rows[0]) || null }))
        .catch(e => { console.error('[businesses] get', e); res.status(500).json({ status: 500, error: 'Failed to load business' }); });
};

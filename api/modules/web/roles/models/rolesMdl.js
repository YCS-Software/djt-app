...
exports.getByIdMdl = function(data) {
    const code = sqldb.MySQLConPool.escape(String(data.id == null ? '' : data.id));
    const QRY = `SELECT usr_typ_cd AS id, usr_typ_cd AS name, CONCAT('System role: ', usr_typ_cd) AS description, COUNT(*) AS users, 'Active' AS status FROM usr_lst_t WHERE usr_typ_cd = ${code} GROUP BY usr_typ_cd`;
    return dbutil.execQuery(sqldb.MySQLConPool, QRY, cntxtDtls);
};

...
exports.getByIdMdl = function (data) {
    ...
    const QRY = `SELECT * FROM ( SELECT t.tkn_id AS id, u.nm_tx AS user, t.tkn_typ_cd AS type, t.expry_ts AS expiresAt, CASE WHEN t.is_rvkd_in=1 THEN 'Revoked' WHEN t.a_in=1 THEN 'Active' ELSE 'Inactive' END AS status, t.i_ts AS createdAt FROM tkn_lst_t t LEFT JOIN usr_lst_t u ON u.usr_id=t.usr_id ORDER BY t.tkn_id DESC ) q WHERE q.id = ${id}`;
    return dbutil.execQuery(sqldb.MySQLConPool, QRY, 'cardsMdl');
};

...
-- Users table
CREATE TABLE usr_lst_t (
    usr_id INT PRIMARY KEY AUTO_INCREMENT,
    phn_nmbr_tx VARCHAR(15) NOT NULL UNIQUE,
    eml_tx VARCHAR(100) UNIQUE,
    nm_tx VARCHAR(100),
    prfl_img_tx VARCHAR(255),
    usr_typ_cd VARCHAR(20) DEFAULT 'customer' COMMENT 'customer, owner, admin, operator',
...
CREATE TABLE trxn_lst_t (
    ...
    a_in TINYINT(1) DEFAULT 1,
    i_ts TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (wllt_id) REFERENCES wllt_lst_t(wllt_id) ON DELETE CASCADE,
    FOREIGN KEY (usr_id) REFERENCES usr_lst_t(usr_id) ON DELETE CASCADE,
...
-- Payment orders (Razorpay order/payment audit trail for wallet top-ups)
CREATE TABLE pay_ordr_lst_t (
...
CREATE TABLE sttn_lst_t (
    sttn_id INT PRIMARY KEY AUTO_INCREMENT,
    ownr_usr_id INT NULL COMMENT 'Station owner (FK usr_lst_t); NULL = platform-owned',
    aprvl_sttus_cd VARCHAR(20) DEFAULT 'active' COMMENT 'active, pending, rejected, suspended',
    ...
    INDEX idx_active (a_in),
    INDEX idx_owner (ownr_usr_id),
    INDEX idx_approval (aprvl_sttus_cd),
...
CREATE TABLE mchn_lst_t (
    ...
    lst_hb_ts TIMESTAMP NULL COMMENT 'Last OCPP heartbeat/boot timestamp',
    a_in TINYINT(1) DEFAULT 1,
    i_ts TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    u_ts TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE sssn_lst_t (
    ...
    ocpp_txn_id_tx VARCHAR(100) NULL COMMENT 'OCPP transactionId (charger-initiated sessions)',
    a_in TINYINT(1) DEFAULT 1,
    i_ts TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    u_ts TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (usr_id) REFERENCES usr_lst_t(usr_id) ON DELETE CASCADE,
    FOREIGN KEY (sttn_id) REFERENCES sttn_lst_t(sttn_id) ON DELETE CASCADE,
    FOREIGN KEY (cnntr_id) REFERENCES cnntr_lst_t(cnntr_id),
    FOREIGN KEY (wllt_trxn_id) REFERENCES trxn_lst_t(trxn_id),
    INDEX idx_user (usr_id),
    INDEX idx_station (sttn_id),
    INDEX idx_status (sttus_cd),
    INDEX idx_date (i_ts),
...
CREATE TABLE sssn_log_lst_t (
    ...
    FOREIGN KEY (sssn_id) REFERENCES sssn_lst_t(sssn_id) ON DELETE CASCADE,
...
CREATE TABLE rvw_lst_t (
    ...
    FOREIGN KEY (usr_id) REFERENCES usr_lst_t(usr_id) ON DELETE CASCADE,
    FOREIGN KEY (sttn_id) REFERENCES sttn_lst_t(sttn_id) ON DELETE CASCADE,
    FOREIGN KEY (sssn_id) REFERENCES sssn_lst_t(sssn_id) ON DELETE SET NULL,
    INDEX idx_user (usr_id),
    INDEX idx_station (sttn_id),
...
CREATE TABLE usg_lst_t (
    ...
    i_ts TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (usr_id) REFERENCES usr_lst_t(usr_id) ON DELETE CASCADE,
    FOREIGN KEY (offr_id) REFERENCES offr_lst_t(offr_id) ON DELETE CASCADE,
    FOREIGN KEY (sssn_id) REFERENCES sssn_lst_t(sssn_id),
    INDEX idx_user (usr_id),
...
BEGIN
    IF NEW.sttus_cd = 'completed' AND OLD.sttus_cd != 'completed' THEN
        INSERT INTO stt_lst_t (usr_id, ttl_sssns_nbr, ttl_enrgy_kwh, ttl_spnt_amt, co2_svd_kg)
...
CREATE INDEX idx_session_user_date ON sssn_lst_t(usr_id, i_ts DESC);
CREATE INDEX idx_transaction_user_date ON trxn_lst_t(usr_id, i_ts DESC);
CREATE INDEX idx_station_location_active ON sttn_lst_t(ltde_nbr, lngtde_nbr, a_in);
...
-- Sample app settings
INSERT INTO sttng_lst_t (sttng_ky_tx, sttng_vl_tx, dscrptn_tx) VALUES

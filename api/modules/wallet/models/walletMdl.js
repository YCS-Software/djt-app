...
exports.createTransactionMdl = function(data) {
    const referenceId = data.referenceId ? `'${data.referenceId}'` : 'NULL';
    const referenceType = data.referenceType ? `'${data.referenceType}'` : 'NULL';
    ...
    const QRY_TO_EXEC = `INSERT INTO wallet_transactions_t 
        (wllt_id, usr_id, trxn_typ_cd, trxn_ctgry_cd, amt, 
         ...
         pymnt_mthd_cd, pymnt_dtls_json, rfrnce_id, rfrnce_typ_cd, 
         sttus_cd, a_in) 
        VALUES 
        (${data.walletId}, ${data.userId}, '${data.type}', '${data.category}', 
         ${data.amount}, ${data.balanceBefore}, ${data.balanceAfter}, 
         '${data.description}', ${paymentMethod}, ${paymentDetails}, 

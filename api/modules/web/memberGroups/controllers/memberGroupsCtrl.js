/**
 * Web Member Groups Controller
 * Admin web console (djt-web) — member groups / cohorts.
 * No backing table yet — list returns []; writes are no-ops.
 */

const std = require(appRoot + '/utils/standardMessages');
const mdl = require('../models/memberGroupsMdl');

exports.list = function(req, res) {
    mdl.listMdl()
        .then(rows => res.status(200).json({ status: 200, rows: rows || [] }))
        .catch(e => { console.error('[memberGroups] list', e); res.status(500).json({ status: 500, error: 'Failed to load member groups' }); });
};

exports.get = function(req, res) {
    mdl.getByIdMdl({ id: req.params.id })
        .then(rows => res.status(200).json({ status: 200, row: (rows && rows[0]) || null }))
        .catch(e => { console.error('[memberGroups] get', e); res.status(500).json({ status: 500, error: 'Failed to load member groups' }); });
};

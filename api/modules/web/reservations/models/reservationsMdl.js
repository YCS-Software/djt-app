...
exports.getByIdMdl = function(data) {
    ...
    const QRY_TO_EXEC = `SELECT * FROM ( SELECT b.bkng_id AS id, b.bkng_cd AS code, u.nm_tx AS user, s.sttn_nm_tx AS station, b.bkng_dte AS date, b.bkng_tm AS time, b.sttus_cd AS status, b.cncltn_rsn_tx AS cancelReason FROM bkng_lst_t b LEFT JOIN usr_lst_t u ON u.usr_id=b.usr_id LEFT JOIN sttn_lst_t s ON s.sttn_id=b.sttn_id ORDER BY b.bkng_id DESC ) q WHERE q.id = ?`;
